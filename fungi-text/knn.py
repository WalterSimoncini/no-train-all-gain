...
def main(args):
    ...
    if args.train_size is None:
        ...
        if args.train_size > 1.0:
            # If the train size is more than 1.0 we assume it represents
            # the number of unique samples per class to be used.
            num_classes = np.unique(train_targets).shape[0]
            args.train_size = int(args.train_size) * num_classes

        # Select only a stratified subset of the training dataset
        stratified_samples = StratifiedShuffleSplit(
            n_splits=1,
            train_size=args.train_size,
            random_state=args.seed
        )

        training_indices, _ = list(stratified_samples.split(train_targets, train_targets))[0]
...
if __name__ == "__main__":
    ...
    # Few show evaluation
    parser.add_argument("--train-size", type=float, required=False, default=None, help="Percentage of the training set used to fit the k-NN model. If > 1 it's considered to be the absolute number of samples per class.")
